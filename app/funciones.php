...
function volcarDatostxt($tvalores){
    ...
    if (!is_writeable(FILEUSER) ){
        // El directorio donde se crea tiene que tener permisos adecuados
        $fich = @fopen(FILEUSER,"w") or die ("Error al crear el fichero.");
        fclose($fich);
    }
    $fich = @fopen(FILEUSER, 'w') or die("ERROR al abrir fichero de usuarios"); // abrimos el fichero para lectura
    for($i=0; $i<count($tvalores); $i++){
        fwrite($fich, $tvalores[$i][0]."|".$tvalores[$i][1]."|".$tvalores[$i][2]."|".$tvalores[$i][3]."\n");
    }
    fclose($fich);
}
...
function volcarDatosjson($tvalores){
    ...
    fwrite($fich, json_encode($tvalores));
